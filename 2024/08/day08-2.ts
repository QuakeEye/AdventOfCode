...
for (let i = 0; i < occCharacterTypes.length; i++) {
    for (let j = 0; j < occCharacters.length; j++) {
        ...
        for (let k = 0; k < occCharacters.length; k++) {
            ...
            // Take width as that will guarantee to go over every cell within the bounds
            for (let l = -width; l <= width; l++) {
                const antinode : [number, number] = [
                    occCharacters[j][1][0] + posDiffX * l, 
                    occCharacters[j][1][1] + posDiffY * l
                ];

                if (!outOfGrid(antinode[0], antinode[1]) && !antiIncludes(antinode))
